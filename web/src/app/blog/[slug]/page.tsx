...
// Pre-render every post at build time for fast, crawlable static pages.
...
export default async function BlogPostPage({ params }: Params) {
  ...
  // Structured data so the post is eligible for rich results.
  const articleLd = {
      ...
      <main style={{ position: "relative", zIndex: 1 }}>
        ...
        <section style={{ borderBottom: "1px solid var(--line)" }}>
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "44px 24px 36px" }}>
            <Reveal>
              <Link href="/blog" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--muted)", textDecoration: "none", fontSize: 13, fontWeight: 600 }}><ArrowLeft size={14} /> All posts</Link>
              <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 12, color: "var(--muted)", margin: "22px 0 12px" }}>
                ...
                <span>·</span><span>{new Date(p.published).toLocaleDateString(undefined, { month: "long", day: "numeric", year: "numeric" })}</span>
              </div>
              <h1 className="cs-display" style={{ margin: 0, fontSize: "clamp(34px, 4.6vw, 44px)", fontWeight: 600, letterSpacing: "-0.02em", lineHeight: 1.15 }}>{p.title}</h1>
              <div style={{ marginTop: 14, fontSize: 13.5, color: "var(--muted)" }}>By {p.author}</div>
            </Reveal>
          </div>
        </section>
        ...
        <article style={{ maxWidth: 720, margin: "0 auto", padding: "36px 24px 72px" }}>
            ...
            <Reveal key={i}>
              <section style={{ marginTop: 38 }}>
                ...
                {s.paragraphs.map((para, j) => (
                  <p key={j} style={{ fontSize: 16, lineHeight: 1.78, color: "var(--foreground)", opacity: 0.88, margin: "0 0 14px" }}>{para}</p>
                ))}
                {s.bullets && (
          ...
          <Reveal>
            <div className="cs-glass" style={{ marginTop: 48, padding: 30, borderRadius: "var(--radius-xl)", textAlign: "center", border: "1.5px solid color-mix(in srgb, var(--accent) 40%, var(--line))" }}>
              <h2 className="cs-display" style={{ margin: "0 0 8px", fontSize: 24, fontWeight: 600 }}>List your next part in seconds</h2>
              ...
              <RevealStagger style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
                {related.map((r) => (
                  <RevealItem key={r.slug} style={{ height: "100%" }}>
                    <Link href={`/blog/${r.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block", height: "100%" }}>
