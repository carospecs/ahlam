...
export async function GET(req: Request) {
  ...
  let parts = (listingRows || []).map((l: any) => {
    ...
    return {
      ...
      photoUrls: Array.isArray(l.photo_urls) ? l.photo_urls.filter(Boolean) : (l.photo_url ? [l.photo_url] : []),
      views: l.views || 0,
  ...
  let vehicles = (vehicleRows || []).map((v: any) => {
    ...
    return {
      ...
      photoUrls: Array.isArray(v.photo_urls) ? v.photo_urls.filter(Boolean) : (v.photo_url ? [v.photo_url] : []),
      shopId: v.shop_id,
      shopName: shop?.name || "Independent seller",
      location: shop?.zip_code || shop?.location || "",
      phone: shop?.business_phone || null,
