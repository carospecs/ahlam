...
export async function POST(req: Request) {
  ...
  const prompt =
    ...
    `Return ONLY JSON: {"vin": "<17 characters>"} if you can read one, or {"vin": null} if none is legible. ` +
    `NEVER invent, guess, or partially fill a VIN.`;

  let vinRaw: string | null = null;
  try {
    ...
    vinRaw = typeof parsed?.vin === "string" ? parsed.vin : null;
  } catch { return NextResponse.json({ ok: false, error: "parse" }, { status: 502, headers: CORS }); }

  const vin = normalizeVin(vinRaw);
  if (!vin) return NextResponse.json({ ok: true, vin: null, vehicle: null }, { headers: CORS });

  // Only surface a VIN NHTSA can actually decode — a misread that isn't a real VIN
  // shouldn't anchor the whole scan. Include a summary the client can show at once.
  const d = await decodeVin(vin);
  if (!d) return NextResponse.json({ ok: true, vin: null, vehicle: null }, { headers: CORS });
