...
// OFF the scan path. The scan prices from the formula (new price × grade discount) and
// stays fast; after results land, the client sends the top few high-value parts here and
// this route researches what comparable used parts actually sold for, via a Gemini
// grounded web search (the same mechanism as lib/pricing.ts's groundedMedianPrice, plus
// comp counts and source citations from the grounding metadata).
//
...
// A result with no sources doesn't render. Bounds: ≤ 5 parts per call, 1 grounded call
// per part, flash only. Failures return nothing for that part — the formula price stands.
...
// One grounded search per part: median sold price + comp count, sources from the
// grounding metadata. Null when grounding finds nothing usable.
  ...
  try {
    const res = await geminiGenerate("gemini-2.5-flash", {
      contents: [{ role: "user", parts: [{ text:
        `Use Google to find what this USED auto part ACTUALLY SOLD FOR recently: "${query}"${spec ? ` (${spec})` : ""}. ` +
    ...
    // Provenance from the grounding metadata — a number with no sources doesn't ship.
