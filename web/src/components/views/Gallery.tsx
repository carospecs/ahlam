...
export function Gallery() {
  ...
  const groups = React.useMemo(() => {
    ...
    const m = new Map<string, { key: string; name: string; photos: { url: string; label: string }[] }>();
    const ensure = (key: string, name: string) => { if (!m.has(key)) m.set(key, { key, name, photos: [] }); return m.get(key)!; };
    const seen = new Set<string>();
    const push = (key: string, name: string, url: string, label: string) => {
      if (!url || seen.has(key + url)) return; seen.add(key + url);
      ensure(key, name).photos.push({ url, label });
    };
    for (const v of vehicles || []) {
      const imgs = Array.isArray(v.images) ? v.images : (v.image ? [v.image] : []);
      imgs.filter(Boolean).forEach((u: string) => push(v.id, vehLabel(v), u, "Whole car"));
    }
    for (const l of listings || []) {
      if (!l.image) continue;
      const v = l.vehicleId ? vById.get(l.vehicleId) : null;
      push(v ? v.id : "__other", v ? vehLabel(v) : "Other parts", l.image, l.part || "Part");
    }
      ...
      {groups.map((g) => (
        <div key={g.key}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(132px, 1fr))", gap: 10 }}>
            {g.photos.map((p, i) => (
              <button key={i} onClick={() => setLb({ images: g.photos.map((x) => x.url), i })} title={p.label} style={{ padding: 0, border: "1px solid var(--line)", borderRadius: 12, overflow: "hidden", cursor: "zoom-in", background: "var(--surface2)", aspectRatio: "1", position: "relative", display: "block" }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.url} alt={p.label} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                <span style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "14px 8px 6px", fontSize: 11, fontWeight: 600, color: "#fff", textAlign: "left", background: "linear-gradient(transparent, rgba(7,11,22,0.82))", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{p.label}</span>
              </button>
            ))}
          </div>
        </div>
