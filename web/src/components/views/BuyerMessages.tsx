...
import { Inbox, Send, Store, MessageCircle, Search, X, Trash2, Paperclip } from "lucide-react";
import { fileToJpegDataUrl } from "@/lib/image";
import { fmtMsgTime, type MsgAttachment } from "@/lib/message-utils";
import { csToast, csConsumePendingThread } from "../Dashboard";

interface BMsg { from: string; text: string; time: string; at?: string | null; attachments?: MsgAttachment[] }
interface BThread {
  id: string; shopId: string; shopName: string; phone: string | null;
  part: string; market: string; time: string; unread?: number; lastAt?: string | null; messages: BMsg[]; lastText: string;
}
...
export function BuyerMessages() {
  ...
  const [photos, setPhotos] = React.useState<{ name: string; file: File; url: string }[]>([]);
  ...
  React.useEffect(() => { load(); const iv = setInterval(load, 8000); return () => clearInterval(iv); }, [load]);
  ...
  async function send() {
    if (!t || sending || (!draft.trim() && !photos.length)) return;
    ...
    setDraft(""); setPhotos([]);
    setSending(true);
    // Optimistic echo — local previews render instantly; the next poll swaps in
    // the stored copy with public URLs.
    setThreads((prev) => prev.map((x) => x.id === t.id
      ? { ...x, messages: [...x.messages, { from: "me", text, time: "Now", at: new Date().toISOString(), attachments: toSend.map((ph) => ({ url: ph.url, name: ph.name })) }], lastText: text || "📷 Photo" }
      : x));
    try {
      const images = await Promise.all(toSend.map((ph) => fileToJpegDataUrl(ph.file)));
      const r = await fetch("/api/messages/mine", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ conversationId: t.id, body: text, ...(images.length ? { images } : {}) }),
      });
        ...
        <div style={{ display: "flex", flexDirection: "column", minWidth: 0, minHeight: 0 }}>
            ...
            {t.messages.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.from === "me" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "70%", padding: "10px 14px", borderRadius: 14, fontSize: 13.5, lineHeight: 1.45, background: m.from === "me" ? "var(--accent)" : "var(--surface2)", color: m.from === "me" ? "#fff" : "var(--foreground)", borderBottomRightRadius: m.from === "me" ? 4 : 14, borderBottomLeftRadius: m.from === "me" ? 14 : 4 }}>
                  ...
                  <div style={{ fontSize: 10.5, opacity: 0.7, marginTop: 4, textAlign: "right" }}>{fmtMsgTime(m.at, m.time)}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "10px 14px 0" }}>
              {photos.map((ph, i) => (
                <span key={i} style={{ position: "relative", display: "inline-block" }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={ph.url} alt={ph.name} style={{ width: 52, height: 52, borderRadius: 9, objectFit: "cover", border: "1px solid var(--line)" }} />
                  <button onClick={() => setPhotos((prev) => prev.filter((_, idx) => idx !== i))} style={{ position: "absolute", top: -6, right: -6, width: 18, height: 18, borderRadius: 999, border: "none", background: "rgba(7,11,22,0.75)", display: "grid", placeItems: "center", cursor: "pointer" }}><X size={11} color="#fff" /></button>
                </span>
              ))}
            </div>
          )}
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: 14, borderTop: "1px solid var(--line)" }}>
            <input ref={fileRef} type="file" accept="image/*" multiple hidden onChange={(e) => {
              ...
              if (files.length) setPhotos((prev) => [...prev, ...files.map((f) => ({ name: f.name, file: f, url: URL.createObjectURL(f) }))]);
              e.target.value = "";
            }} />
            <button onClick={() => fileRef.current?.click()} title="Attach photos" style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid var(--line)", background: "transparent", display: "grid", placeItems: "center", flexShrink: 0, cursor: "pointer" }}><Paperclip size={17} color="var(--muted)" /></button>
            <input
              value={draft}
            ...
            />
            <button disabled={sending || (!draft.trim() && !photos.length)} onClick={send} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "10px 16px", borderRadius: 10, border: "none", background: "var(--accent)", color: "#fff", fontSize: 13.5, fontWeight: 700, cursor: "pointer", opacity: sending || (!draft.trim() && !photos.length) ? 0.6 : 1 }}><Send size={15} /> Send</button>
          </div>
        </div>
      )}
    </div>
