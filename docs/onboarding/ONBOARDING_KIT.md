...
Last updated: 2026-07-05 (refreshed for the market-comps pricing system and current channel lineup; welcome email now sent automatically on shop creation)
...
- **Pricing comes from real market comps.** The app retrieves live eBay listings for the same part, a judge model filters junk comps and anchors complete assemblies, and the suggested price carries **evidence-based confidence** (`lib/market-pricing`, `lib/used-pricing`). The honest line is still: "it's a smart starting price you review," backed by real listings rather than a formula.
...
- **Facebook Marketplace and OfferUp have no posting API.** Ahlam copies the listing text and saves the photos, then opens the post form. **Craigslist and the Car-Part.com / URG feed are in development ("coming soon" in the app since Jul 2026) — do not promise them to customers yet. DoorDash local delivery is also planned.** The optional **Chrome extension ("Ahlam Auto-Poster")** goes one step further and auto-FILLS title, price, description, and photos into those forms. **It never auto-publishes — the user always reviews and clicks Publish.** On a phone, there's no extension; the app uses the OS share sheet to hand off to the marketplace app.
- **Bulk export via CSV:** a general listings CSV and a Facebook/Meta catalog CSV. (The Car-Part.com / URG inventory CSV is built but disabled in the UI until that channel launches.)
...
As of 2026-07-05 this email is sent AUTOMATICALLY when a shop is created (`api/onboarding/route.ts` → `lib/onboarding-email.ts`), from andygarcia@ahlam.io with mohammadabbas@ahlam.io CC'd. Edit `lib/onboarding-email.ts` to change what customers receive; this doc is the reference copy.
...
- Want to post to Facebook or OfferUp with the fields filled in for you on your computer? Install the free Ahlam Auto-Poster (one-time, ~30 seconds). It fills the form; you always review and click Publish yourself. Link is in the app under Export & posting.
...
4. **Publícala.** Un clic a eBay, o copiar y pegar a Facebook y OfferUp. (Craigslist, Car-Part.com y la entrega con DoorDash llegan pronto.)
...
- [ ] **Post it.**
  ...
  - Facebook / OfferUp: click **Post elsewhere**, and either paste it in yourself or let the Ahlam Auto-Poster fill the form for you (you click Publish).
